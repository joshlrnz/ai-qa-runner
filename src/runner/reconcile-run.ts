...
async function readPlaywrightOutcome(runId: string) {
  const lastRunPath = path.join(getRunDirectory(runId), 'test-results', '.last-run.json')
...
async function getReportUrl(runId: string) {
  return (await hasReport(runId)) ? `/api/runs/${runId}/report/index.html` : null
}
...
export async function reconcileRun(run: TestRun): Promise<TestRun> {
  ...
  if (outcome) {
    const reconciled: TestRun = {
      ...
      error:
        outcome === 'passed'
          ? null
          : 'The run finished while the server was not watching it. Open the report for the failed step.',
  ...
  const isAbandoned = Number.isFinite(startedAt) && Date.now() - startedAt > abandonedRunTimeoutMs
  ...
  const abandoned: TestRun = {
    ...
    status: 'failed',
    updatedAt: new Date().toISOString(),
    error: 'The run never reported a result and has been abandoned. See run.log for details.',
    reportUrl: await getReportUrl(run.runId)
  }
